...
unsigned int CalibrateOutput()
{
	...
	fprintf(stderr, "Fillstart is %i %llu\n", n_start, TimeBefore);
	...
	fprintf(stderr, "Fillstop is %i %llu\n", n, _timestamp_ns());
	unsigned int OutSampleRate = (WaitSampleWritten)*1e9 / (_timestamp_ns() - TimeBefore);

    if (ret == 0)
    {
      //GKQ code to prevent variable ‘ret’ set but not used warning
    }
...
bool RunWithFile(bool live)
{
	...
	if (live)
	{
        ...
        if (ret2 == 0)
        {
          //GKQ code to prevent variable ‘ret2’ set but not used warning
        }
